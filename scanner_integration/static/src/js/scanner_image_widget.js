...
function compressPageImage(b64, maxW = PAGE_MAX_W, maxH = PAGE_MAX_H, quality = PAGE_JPEG_Q) {
...
/**
 * Rotate a base64 image by `degrees` (90 / -90 / 180).
 * Returns a new base64 PNG.  The canvas is resized so nothing is clipped.
 */
...
class ScannerImageField extends Component {
        ...
        return new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(() => {
                ...
                settle(reject, new Error(
                    ...
                    `Run:  python cheque_scanner_bridge.py`
                ));
            }, CONNECT_TIMEOUT);
            ...
            ws.onerror = () => {
                ...
                settle(reject, new Error(
                    ...
                    `Start it with:  python cheque_scanner_bridge.py`
                ));
            };
        });
    }
    ...
    async onScanClick() {
        ...
        try {
            ...
            const imageB64 = await new Promise((resolve, reject) => {
                ...
                const cleanup = (restoreClose = true) => {
                    clearTimeout(timer);
                    ws.onmessage = null;
                    ws.onerror   = null;
    ...
    onUploadClick() { this.fileInputRef.el?.click(); }
...
registry.category("fields").add("scanner_image", {
...
class ScannerPdfField extends Component {
    ...
    static supportedTypes = ["many2many"];
    static props          = { "*": true };
    ...
    _makeThumbnail(b64, maxW = 150, maxH = 200) {
        return new Promise((resolve) => {
            const mime = _b64Mime(b64);
            const img  = new Image();
            img.onload = () => {
    ...
    onDirectUploadClick() { this.fileDirectRef.el?.click(); }
    ...
    async _doScanFlatbed(scannerName = null) {
        if (this.state.scanning) return;
        this.state.scanning   = true;
    ...
    async _doScanAdf(scannerName = null) {
        if (this.state.scanning) return;
        this.state.scanning   = true;
        ...
        try {
            ...
            setTimeout(() => { this.state.statusMsg = ""; }, 4_000);
    ...
    async onFileChange(ev) {
        ...
        if (files.length === 1) {
            ...
            setTimeout(() => { this.state.statusMsg = ""; }, 3_000);
        }
...
registry.category("fields").add("scanner_pdf", {
