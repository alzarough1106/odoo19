...
class ScannerImageField extends Component {
    ...
    setup() {
        ...
        // Bind methods called with arguments from the template
...
ScannerImageField.supportedTypes = ["binary"];
ScannerImageField.props          = { "*": true };
ScannerImageField.extractProps   = ({ attrs } = {}, dynamicInfo) => {
    const a = attrs || {};
    return {
        readonly: !!(dynamicInfo && dynamicInfo.readonly) ||
                  a.readonly === "1" || a.readonly === "True",
    };
};
...
class ScannerPdfField extends Component {
    ...
    setup() {
        ...
        // Bind all methods called with arguments from the template
    ...
    async onRemoveAttachment(id) {
        ...
        try {
            ...
            console.warn("ScannerPdfField: could not delete attachment", id, e);
        }
    }
    ...
    async _doSavePdf(filename) {
        ...
        try {
            ...
            // ✅ FIX 1: orm.create returns a plain integer in Odoo 16, not an array
    ...
    async onDirectFileChange(ev) {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                // ✅ FIX 2: same issue — orm.create returns an integer, not an array
                const attachId = await this.orm.create("ir.attachment", [{
                    name     : file.name,
                    type     : "binary",
                    datas    : b64,
...
ScannerPdfField.supportedTypes = ["many2many"];
ScannerPdfField.props          = { "*": true };
ScannerPdfField.extractProps   = ({ attrs } = {}, dynamicInfo) => {
    const a = attrs || {};
    return {
        readonly: !!(dynamicInfo && dynamicInfo.readonly) ||
                  a.readonly === "1" || a.readonly === "True",
